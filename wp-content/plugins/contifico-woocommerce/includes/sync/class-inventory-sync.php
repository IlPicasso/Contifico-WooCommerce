...
class Contifico_WooCommerce_Sync_Inventory_Sync {
    ...
    protected function process_inventory_response( array $warehouses, array $args = array() ) {
        ...
        return array(
            ...
            'product_map'            => $built_inventory['product_map'],
    ...
    protected function build_inventory_map( array $warehouses, $timestamp ) {
        ...
        foreach ( $warehouses as $warehouse ) {
            ...
            foreach ( $items as $item ) {
                ...
                if ( isset( $item['contifico_id'] ) && '' !== $item['contifico_id'] && ! isset( $product_map[ $product_id ] ) ) {
                    $product_map[ $product_id ] = array(
                        'contifico_id' => (string) $item['contifico_id'],
                    );
                }
    ...
    /**
     * Guarda metadatos adicionales como el identificador de producto de Contifico.
     *
     * @param array $product_map Mapa de producto a metadatos.
     *
     * @return void
     */
    protected function persist_product_metadata( array $product_map ) {
        foreach ( $product_map as $product_id => $data ) {
            $product_id = absint( $product_id );

            if ( ! $product_id || empty( $data['contifico_id'] ) ) {
                continue;
            }

            update_post_meta( $product_id, '_contifico_product_id', (string) $data['contifico_id'] );
        }
